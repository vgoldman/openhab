...
public class ZWaveSecureNonceTracker {
	...
	/**
	 * Nonces generated by us and sent to the device in a {@link ZWaveSecurityCommandClass#SECURITY_NONCE_REPORT} message
	 * Used in the decryption process to process incoming SECURITY messages
	 */
	private NonceTable ourNonceTable = new NonceTable();

	/**
	 * Nonces generated by the device and sent to us in a {@link ZWaveSecurityCommandClass#SECURITY_NONCE_REPORT} message.
	...
	synchronized SerialMessage buildNonceGetIfNeeded() {
		...
		// We only try once as strange things happen with NONCE_GET requests TODO: DB add more detail as to what we are trying to fix here
		...
		private void cleanup() {
			...
			while(iter.hasNext()) {
				...
				if(nonce.getTimer() != null && nonce.getTimer().isExpired()) {
					logger.warn(String.format("NODE %s: Expiring nonce with id=%02X",
							node.getNodeId(), nonce.getNonceId()));
					iter.remove();
