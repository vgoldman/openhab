...
public class SendDataMessageClass extends ZWaveCommandProcessor {
	...
	public boolean handleRequest(ZWaveController zController, SerialMessage lastSentMessage,
		...
		final int callbackId = incomingMessage.getMessagePayloadByte(0);
		final TransmissionState status = TransmissionState.getTransmissionState(incomingMessage.getMessagePayloadByte(1));
		...
		final ZWaveNode node = zController.getNode(lastSentMessage.getMessageNode());
		if(node == null) {
		...
		case COMPLETE_NO_ACK:
			...
			final byte[] payload = lastSentMessage.getMessagePayload();
			if (payload.length >= 4 &&
	...
	public boolean handleFailedSendDataRequest(ZWaveController zController, SerialMessage originalMessage) {
		...
		final ZWaveNode node = zController.getNode(originalMessage.getMessageNode());
		...
		if (!node.isListening() && !node.isFrequentlyListening()) {
			final ZWaveWakeUpCommandClass wakeUpCommandClass = (ZWaveWakeUpCommandClass) node
		...
		logger.error("NODE {}: Got an error while sending data. Resending message: {}", node.getNodeId(), originalMessage);
	...
	public enum TransmissionState {
		...
		private static void initMapping() {
			...
			for (final TransmissionState s : values()) {
