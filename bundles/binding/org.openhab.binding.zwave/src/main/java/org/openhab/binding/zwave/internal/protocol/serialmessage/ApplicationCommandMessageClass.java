	...
	public  boolean handleRequest(ZWaveController zController, SerialMessage lastSentMessage, SerialMessage incomingMessage) {
		...
		if(zwaveCommandClass instanceof ZWaveSecurityCommandClass
			...
			if(decryptedBytes == null) {
				...
				// call handleApplicationCommandRequest with the decrypted message
				// Note that we do NOT set incomingMessage as that needs to be processed below
				// with the original security encapsulated message
			...
			if(node.doesMessageRequireSecurityEncapsulation(incomingMessage)) {
				...
				logger.error("NODE {}: Command Class {} {} was required to be security encapsulation but it wasn't!  Dropping message.",
						nodeId,	zwaveCommandClass.getCommandClass().getKey(), zwaveCommandClass.getCommandClass().getLabel());
