...
public class GoogleTtsActionService implements ActionService, ManagedService {
    ...
    public void updated(Dictionary<String, ?> config) throws ConfigurationException {
        ...
        if (!dir.exists()) {
            logger.debug(
                    "GoogleTts Cache directory does not exist, creating directory: " + GoogleTts.GOOGLETTS_CACHE_PATH);
        ...
        if (dir.isDirectory()) {
            ...
            if (afile.length > 0) {
                for (int i = 0; i < afile.length; i++) {
                    if (afile[i].isFile()) {
                        logger.debug("Adding cached voice file: " + afile[i].getName());
                    }
        ...
        logger.debug("Google TTS is properly configured now with defaultLang:" + GoogleTts.defaultLang
                + " and cache enabled:" + GoogleTts.isCacheEnabled);
