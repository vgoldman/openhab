...
public class GoogleTts {
    ...
    public static final String GOOGLETTS_URL_TEMPLATE = "http://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&prev=input&q=%s&tl=%s";

    private static final Logger logger = LoggerFactory.getLogger(GoogleTts.class);
    ...
    public static synchronized boolean voice(@ParamDoc(name = "text", text = "Text to be said") String text,
        ...
        for (int end = iterator.next(); end != BreakIterator.DONE; start = end, end = iterator.next()) {
            ...
            if (isCacheEnabled && GoogleTtsActionService.textAudioFiles.contains(hashFilename)) {

                logger.debug("GoogleTTS text found in cache and will be played from file: " + hashFilename);
                playTextAudioFromCache(hashFilename);
                logger.debug("GoogleTTS Finished playing segment");

            } else {
                logger.debug("GoogleTTS cacheEnabled " + isCacheEnabled + " or cached voice not found.");
                try {
                    sentenceEncoded = URLEncoder.encode(sentenceText, "UTF-8");
                    logger.debug("GoogleTTS encoded sentence:" + sentenceEncoded);
                } catch (UnsupportedEncodingException e) {
                    logger.error("GoogleTTS could not encode" + sentenceEncoded, e);
                    return false;
                }
                String urlString = String.format(GOOGLETTS_URL_TEMPLATE, sentenceEncoded, lang);
    ...
    private static void playTextAudioFromCache(String hashFilename) {
        ...
        if (!textAudioFile.exists()) {
            ...
            GoogleTtsActionService.textAudioFiles.remove(hashFilename);
            return;
        }
        logger.debug("GoogleTTS found voice file on disk: " + hashFilename);
        ...
        try {
            ...
            logger.error("Could not play the file. See exception trace for details", e);
        } catch (IOException e) {
            logger.error("Could not play the file. See exception trace for details", e);
        } finally {
            if (in != null) {
    ...
    private static void saveVoiceAudioToFile(BufferedInputStream in, String hashFilename) throws IOException {
        OutputStream outputStream = new FileOutputStream(GOOGLETTS_CACHE_PATH + hashFilename);
